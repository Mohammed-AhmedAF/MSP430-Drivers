...
void GPIO_vidSetPinResisPullUpDown(u8 u8PortNumber, u8 u8PinNumber,
                                   u8 u8ResistanceFlag)
{
    switch (u8PortNumber)
    {
...
void GPIO_vidConfigPinInterrupt(u8 u8PortNumber, u8 u8PinNumber, u8 u8Edge)
{
    ...
    {
    case GPIO_PORT1:
        if (u8Edge == GPIO_EDGE_HL)
        {
            SET_BIT(P1IES, u8PinNumber);
        }
        else
        {
            CLEAR_BIT(P1IES, u8PinNumber);
        }
        break;
    case GPIO_PORT2:
        if (u8Edge == GPIO_EDGE_HL)
        {
            SET_BIT(P2IES, u8PinNumber);
        }
        else
        {
            CLEAR_BIT(P2IES, u8PinNumber);
        }
        break;
    }
}
...
void GPIO_vidEnablePinInterrupt(u8 u8PortNumber, u8 u8PinNumber)
{
    switch (u8PortNumber)
       {
       case GPIO_PORT1:
               SET_BIT(P1IE, u8PinNumber);
           break;
       case GPIO_PORT2:

               SET_BIT(P2IE, u8PinNumber);
           break;
       }
}
